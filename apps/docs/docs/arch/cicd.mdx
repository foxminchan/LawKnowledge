...
We use Jenkins as a Docker container. The following command will pull the latest Jenkins image from Docker Hub and run it on port 8080.
...
After running the above command, we can access Jenkins at http://localhost:8080. The first time we access Jenkins, we will be asked to enter the administrator password. We can get the password by running the following command.
...
Spinnaker is an open-source, multi-cloud continuous delivery platform for releasing software changes with high velocity and confidence. It provides two core sets of features: cluster management and deployment management. Spinnaker’s deployment management features include:
...
We use Spinnaker as a Helm chart. The following command will add the Spinnaker Helm chart repository.
...
The following command will install Spinnaker.
...
The following command will forward the Spinnaker UI port to port 9000.
